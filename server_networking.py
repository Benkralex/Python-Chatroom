...
def log(message, log_type='INFO'):
    full_msg = f'[{str(datetime.datetime.now())}] [{str(log_type)}] {str(message)}\n'
    print(full_msg)
...
class ServerNetworking:
    ...
    def handle_client(self, client_socket):
        try:
            ...
            self.broadcast(f'{client_name} hat den Chat betreten.'.encode('utf-8'))
    ...
    def send_users_list(self):
        ...
        self.broadcast(users_list.encode('utf-8'))